...
A página principal, exibida na \autoref{fig:agenda-diaria-terca}, é a primeira página que um novo usuário vê, por padrão. 
...
No modo de agenda diária, exemplificado na \autoref{fig:agenda-diaria-sabado}, é apresentado a data atual e uma grade, contendo todos os eventos que acontecem neste dia, organizados por sua hora de início. 
...
Ao alternar para o modo de agenda semanal, como na \autoref{fig:agenda-semanal}, é apresentado uma outra grade, onde são apresentados todos os eventos que ocorrem na semana da data atual, e também o domingo da próxima semana, a fim de exibir todo o final de semana.
...
Há também duas caixas de seleção, onde é possível filtrar os eventos exibidos nesta página por suas categorias ou regiões, conforme demonstrado na \autoref{fig:agenda-semanal-filtros}. 
...
É possível observar alguns na \autoref{fig:busca-filtros}.
...
Realizar uma busca nesta página exibe uma lista de resultados, eventos cujas características se enquadram nos parâmetros informados pelo usuário, conforme a figura \autoref{fig:busca-realizada}.
...
A página de evento, que é acessada por meio de divesas outras páginas da aplicação, exibe as informações de um determinado evento, tal qual exemplificado na \autoref{fig:evento}.
...
Ainda assim, a página evidencia o componente de login ou cadastro, dependendo de qual \emph{link} foi clicado.

O componente de cadastro, mostrado na \autoref{fig:cadastro-pessoa}, contém um formulário, com os campos nome, email, e senha, e enviá-lo efetua a criação de um novo usuário, do tipo Pessoa, no sistema. Também existe um botão que indica um modo de cadastro como Organizador. 
Clicá-lo faz com que enviar este formulário apresente um novo campo para um CPF ou CNPJ (evidenciado na \autoref{fig:cadastro-organizador}), e enviá-lo cria um novo usuário do tipo Organizador, ao invés de Pessoa.
...
O componente de login, exibido na \autoref{fig:login}, presente na mesma página, permite realizar a operação de autenticação.
...
Na \autoref{fig:perfil-pessoa}, se observa o perfil para um usuário do tipo Pessoa.
...
O Componente de atualização de evento possui um formulário que, ao preenche-lo com um título e uma descrição, e envia-lo, adiciona uma atualização ao evento referente, conforme exemplificado na \autoref{fig:atualizacao-preenchido}.
...
Já a página de exclusão de eventos, representada na \autoref{fig:exclusao-evento}, contém apenas alguns botões: voltar, editar e excluir, que realizam ações sob um determinado evento. 
...
Por fim, a última nova págin a disponível a um Organizador é a de novo evento, exibida na \autoref{fig:novo-evento}. 
...
Ao fim da página, há dois botões, que alteram o status deste evento para aprovado ou reprovado, além de um terceiro botão que leva de volta ao perfil, tal qual mostrado na \autoref{fig:analise-evento}.
...
Finalmente, existe uma página para qualquer URL não previsto pela aplicação, exibido na \autoref{fig:notfound}.
Esta página também é exibida quando se tenta acessar um evento que não pode ser encontrado, ou quando se tenta acessar uma página restrita, exemplificado na \autoref{fig:notauth}.
